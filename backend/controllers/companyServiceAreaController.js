...
const createCompanyServiceArea = asyncHandler(async (req, res) => {
  ...
  const cityExists =
    await CityMaster.findById(city);
  ...
  if (!areaExists) {
    res.status(400);
    throw new Error(
      "Invalid area selected for city"
    );
  }
  ...
  const existing =
    await CompanyServiceArea.findOne({
      company,
      area,
    });
  ...
  if (existing) {
    res.status(400);
    throw new Error(
      "Company already serves this area"
    );
  }
  ...
  const serviceArea =
    await CompanyServiceArea.create({
      company,
      city,
      area,
    });
  ...
  const populated =
    await CompanyServiceArea.findById(
      serviceArea._id
    )
    .populate(
      "company",
...
console.log(req.query);
  ...
  const mappings =
    await CompanyServiceArea.find(query)
 ...
 * @access  Private/Admin
 */
const updateCompanyServiceArea =
asyncHandler(async(req,res)=>{
  ...
  if(!mapping){
    ...
    throw new Error(
      "Service area mapping not found"
    );
 ...
 * @access  Private/Admin
 */
const toggleCompanyServiceAreaStatus =
asyncHandler(async(req,res)=>{
 ...
 * @access  Private/Admin
 */
const deleteCompanyServiceArea =
asyncHandler(async(req,res)=>{


  const mapping =
    await CompanyServiceArea.findByIdAndDelete(
      req.params.id
    );
